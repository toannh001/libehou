...
Class HLDT{
	...
	public static function Timkiemhoclieu($name){
		$url = "http://uni.ehou.edu.vn/api/v1/LCMSNhomHocLieu?orderBy=name&maxSize=100&q=".$name;
		$api=new API();
		$request = $api->WebGet($url );
		return json_decode($request);
	}
	...
	public static function GetDetailBaigiangText($id){
		$url = "http://uni.ehou.edu.vn/api/v1/LCMSHocLieu?maxSize=100&offset=0&order=asc&where[0][type]=in&where[0][attribute]=loaiHocLieu&where[0][value][]=baigiangtext&where[1][type]=contains&where[1][attribute]=id&where[1][value]=".$id;
		$api=new API();
		$request = $api->WebGet($url);
		//print_r(json_decode($request));exit();
		return json_decode($request);
	...
	public static function GetHoclieuByNganh($nganh){
		$url = "http://uni.ehou.edu.vn/api/v1/LCMSNhomHocLieu?maxSize=100&offset=0&orderBy=createdAt&order=desc";
		$api=new API();
		$request = $api->WebGet($url );
		return json_decode($request);
	}
	...
	public function VỉewBaigiangText($fileId){
		$api=new API();
		$request = $api->ViewBaigiangText($fileId);
	}
